...
function applyLanguage(language) {
    ...
    document.querySelectorAll("body *").forEach((element) => {
        if (element.children.length === 0) {
            ...
            const originalText = originalTextMap.get(element).trim();

            if (selectedLanguage === "fil") {
                if (automaticTranslations[originalText]) {
                    element.textContent = automaticTranslations[originalText];
                }
            } else {
    ...
    document.querySelectorAll("input, textarea").forEach((element) => {
        if (!originalPlaceholderMap.has(element)) {
            originalPlaceholderMap.set(element, element.placeholder);
        }

        const originalPlaceholder = originalPlaceholderMap
            .get(element)
            .trim();
        ...
        if (selectedLanguage === "fil") {
            if (
                ...
                originalPlaceholder === "Search applications..."
            ) {
                element.placeholder = "Maghanap ng mga aplikasyon...";
            } else if (
    ...
    localStorage.setItem("valobloxLanguage", selectedLanguage);
}
